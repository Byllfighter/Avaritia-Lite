...
public class EndestPearlItem extends Item {
	public EndestPearlItem() {
		super(new Item.Properties().tab(AvaritiaModTabs.TAB_AVARITIATAB).stacksTo(64));
	...
	public void releaseUsing(ItemStack itemstack, Level world, LivingEntity entityLiving, int timeLeft) {
		if (!world.isClientSide() && entityLiving instanceof ServerPlayer entity) {
			...
			if (true) {
				ItemStack stack = ProjectileWeaponItem.getHeldProjectile(entity, e -> e.getItem() == AvaritiaModItems.ENDEST_PEARL);
				if (stack == ItemStack.EMPTY) {
					for (int i = 0; i < entity.getInventory().items.size(); i++) {
						ItemStack teststack = entity.getInventory().items.get(i);
						if (teststack != null && teststack.getItem() == AvaritiaModItems.ENDEST_PEARL) {
							stack = teststack;
							break;
						}
					}
				...
				if (entity.getAbilities().instabuild || stack != ItemStack.EMPTY) {
					...
					if (entity.getAbilities().instabuild) {
						...
						if (new ItemStack(AvaritiaModItems.ENDEST_PEARL).isDamageableItem()) {
